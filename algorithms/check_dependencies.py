...
opcjonalnych dla algorytmów Vector from Map:
  ...
  • PyTorch        — wymagane dla treningu i inferencji .pth
  • segmentation-models-pytorch — wymagane dla architektur SMP
  • timm           — wymagane dla części encoderów SMP (np. timm-*)
...
class CheckDependenciesAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not torch_ok:
            log('     → Wymagane dla: Trenowanie i inferencja modeli .pth / .pt')
            log('     → Instalacja: https://pytorch.org/get-started/locally/')

        smp_ok = smp_available()
        smp_v = smp_version() or 'niedostępne'
        log(f'  {_status(smp_ok)} segmentation-models-pytorch  {smp_v}')
        if not smp_ok:
            log('     → Wymagane dla: DeepLabV3Plus, LinkNet, FPN, PSPNet, MAnet')
            log('     → Instalacja: pip install segmentation-models-pytorch timm')

        timm_ok = timm_available()
        timm_v = timm_version() or 'niedostępne'
        log(f'  {_status(timm_ok)} timm  {timm_v}')
        if not timm_ok:
            log('     → Wymagane dla encoderów typu "timm-*".')
            log('     → Instalacja: pip install timm')
        ...
        if saga_ok:
            ...
            log('     → 8 algorytmów proxy (klasyfikacja, k-means, przetwarzanie obrazu)')
        ...
        algo_status(
            'Trenowanie modelu — PyTorch (U-Net + SMP)',
            ['torch', 'segmentation-models-pytorch (opcjonalnie dla SMP)'],
            torch_ok,
        )
        ...
        algo_status('Algorytmy proxy SAGA (8 szt. — klasyfikacja, k-means, przetwarzanie obrazu)',
                    ['SAGA NG'], saga_ok)
