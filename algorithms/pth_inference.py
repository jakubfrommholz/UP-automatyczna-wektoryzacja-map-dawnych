...
class PthInferenceAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        return (
            ...
            'lub jako checkpoint state_dict z algorytmu "Trenowanie modelu (PyTorch)" '
            '(U-Net, DeepLabV3Plus, LinkNet, FPN, PSPNet, MAnet).\n'
    ...
    def _load_runner(self, fmt, model_path, feedback):
        ...
        if fmt == 'pytorch':
            ...
            try:
                ...
                if isinstance(model, dict) and 'state_dict' in model and 'model_type' in model:
                    ...
                    encoder_name = model.get('encoder_name') or 'resnet34'
                    try:
                        ...
                        feedback.reportError(
                            f'Nie udało się odtworzyć modelu z checkpointu: {e_restore}',
                            fatalError=True)
                        return None
                    model = restored
                    feedback.pushInfo(
                        f'Wczytano checkpoint {model_type} '
                        f'(C_in={in_channels}, classes={num_classes}, base={base_channels}, '
